...
extension JSController {
    func fetchDetailsJS(url: String, completion: @escaping ([MediaItem], [EpisodeLink]) -> Void) {
        ...
        let promiseValueDetails = extractDetailsFunction.call(withArguments: [url])
                ...
                if let jsonOfDetails = result.toString(),
                   let dataDetails = jsonOfDetails.data(using: .utf8) {
                    do {
                        ...
                        } else if let dict = try JSONSerialization.jsonObject(with: dataDetails, options: []) as? [String: Any] {
                            resultItems = [
                                MediaItem(
        ...
        dispatchGroup.enter()
        let promiseValueEpisodes = extractEpisodesFunction.call(withArguments: [url])
